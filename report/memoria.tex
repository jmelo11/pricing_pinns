...
We begin by briefly reviewing the framework proposed by Black and Scholes \cite{black1973}. As already stated in the introducction, they assumed that the
...
Solving the multi-asset case described adove via the PDE \eqref{eq:scaled_PDE_exact} is a significant challenge due to the complexity
...
\eqref{eq:BS_complete}. The idea is to discretise the time and asset-price domains
...
we implemented in PyTorch, consistently delivered the best outcomes. Gradient-based 
...
In all experiments we report both the rel-\(\ell_2\) error, defined as
\[ \text{rel-}\ell_2\text{ error} = \frac{\|V_{\text{PINN}}- V_{\text{ref}}\|_{\text{Fro}}}{\|V_{\text{exact}}\|_{\text{Fro}}}, \]
and the mean-square error (MSE) over a test set of \(N\) points:
\[ \text{MSE} = \frac{1}{N}\sum_{i=1}^{N} \left(V_{\text{PINN}} - V_{\text{ref}}\right), \]
...
the network reproduced the analytical solution accurately, achieving an rel-\(\ell_2\) error of \(4.19\times10^{-5}\) and a MSE of \(4.05\times10^{-9}\).
...
delta, denoted by $\Delta$, (Figure~\ref{fig:delta}) is recovered almost perfectly,
...
Gamma, denoted by $\Gamma$, poses the greatest challenge (Figure~\ref{fig:gamma}). 
As a second-order derivative of the option price, it is highly sensitive to even small oscillations 
in the reconstructed price surface, which leads to the largest rel-$\ell_{2}$ error observed in 
...
Theta, denoted by $\Theta$, sits between the two extremes: its error ($12\%$) is larger than
...
\begin{table}[htbp]
  ...
  \begin{tabular}{c c c c}
    \toprule
     & $\Delta$ & $\Theta$ & $\Gamma$ \\
    \midrule
    rel-$\ell_{2}$ error & 0.0045 & 0.1165 & 0.2394 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
 ...
 \caption{Dimensionless multi-asset best-of option price (first two asset axes).}
...
\begin{figure}[H]
 ...
 \caption{Multi-asset best-of option price in original coordinates (first two asset axes).}
...
relative rel-\(\ell_2\) error arround \(10^{-3}\) which is generally considered 
...
The bell-shaped profiles arise because volatility in the Heston model follows a mean-reverting process. 
Very high or very low initial volatilities tend to revert towards the long-term mean during the option's life. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{imagenes/hhw_bs_diff_sigma.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{imagenes/hhw_bs_diff_rates.png}
	\caption{Difference between Heston-Hull-White and Black-Scholes call prices 
...
Finally, we note that the relative $\ell_2$ error of the PINN solution is $4.61\times 10^{-3}$ 
and the MSE is $4.98\times 10^{-5}$. These errors are acceptable for practical applications, 
although they are higher than in the simpler Black-Scholes case. 
...
to high accuracy: the relative rel-\(\ell_2\) error of the parity is \(1.40\times10^{-4}\) and a MSE of \(6.75\times10^{-9}\).
...
This modified loss directly encodes the early-exercise constraint by enforcing the 
option value to remain above its intrinsic value $\Phi(S)$ while still satisfying the 
PDE in the continuation region. In other words, the network is trained to respect the 
minimum of the two competing conditions: no-arbitrage through the payoff constraint 
and consistency with the governing PDE. More on this can be found in \cite{wilmott1995mathematics, IKONEN2004809}. 
...
\begin{figure}[H]
	...
	\caption{Absolute error for American put option pricing. Its possible to observe that the PINN is able to reproduce closely most of the free boundary.}
...
prediction of the free boundary, highlited by the blue and yellow lines,
which is the main challenge in American option pricing. Still, some artifacts are visible, especially near the strike price.

In terms of the accuracy of the obtained solution, the PINN achieved an rel-\(\ell_2\) error of 
...
\]
Here, $N(\cdot)$ denotes a standar normal CDF and the parameters are the sames as in Section \ref{sec:sr_extension}.
...
For this particular case, the PINN achieved an rel-\(\ell_2\) error of \(5.7\times10^{-3}\) and a MSE of \(1.15\times10^{-5}\). In general, 
...
\toprule
\(d\) & Layers & Width & Instrument & Model & Training Time (hr) & \(\Omega\) & \(\partial \Omega\) & \(\Omega_0\) & rel-\(\ell_2\) error & MSE \\
\midrule
