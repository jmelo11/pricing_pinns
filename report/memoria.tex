...
payoffs $(V_T,\widehat V_T)$, so the common value of the two quotients can depend \emph{only} on the
...
Before optimization begins, the parameters~$\theta$ are typically initialised randomly, commonly using Xavier \cite{pmlr-v9-glorot10a} or 
He initialisation \cite{he2015delvingdeeprectifierssurpassing}, or drawn from a pre-trained model. Good initialisation accelerates convergence and improves final accuracy. 
...
GitHub \cite{Melo2025pricingpinns} and a brief article on the topic can be found in \cite{melo_options_pinns_2025}. Computations were performed on a personal computer equipped with an Intel i7-10700K CPU and 
...
Figure~\ref{fig:call_option} shows that the learned price surface is visually indistinguishable from the 
...
differentiation is that it uses \emph{exact} derivatives of the network
...
delta ($\partial V / \partial S$), denoted by $\Delta$, is recovered almost perfectly,
...
\begin{figure}[H]
 ...
 \caption{Absolute error of the PINN solution. The error remains small even
	when extrapolating beyond the training domain.}
...
major limitation in professional financial settings. In practice, \emph{model parameters such 
...
\begin{figure}[H]
	...
	particularly when initial rates are low.}
...
where $\Phi(S)$ is the payoff function of the American put option, and $\mathcal{N}_I[\cdot]$ is the differential operator defined in \eqref{eq:PDE_conditions}.
...
The resulting PINN solution, \emph{compared against a benchmark numerical solution--FDM--}
...
prediction of the free boundary, highlighted by the blue and purple lines,
which is the main challenge in American option pricing. Still, some artifacts are visible, especially near maturity.
...
\]
and set \(X_i^{\star}=A(T,T_i)e^{-B(T,T_i)r^{\star}}\). Then, the pricing formula of a \emph{payer swaption} is given by
\[
\operatorname{PS}(t,T,\mathcal{T},X)
        = \sum_{i=1}^{n} c_i 
