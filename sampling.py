...
def random_samples(n_samples, dimension, sampler="pseudo"):
    if sampler == "pseudo":
...
def residual_based_adaptive_sampling(res_f, n_samples, ranges, k, c, sampler="pseudo"):
    """
    ...
    """
    samples = random_samples(n_samples*10, len(ranges), sampler)
    scaled_samples = scale_samples(samples, ranges)
    residual = res_f(scaled_samples)
    ...
    selected_ids = np.random.choice(
        a=len(scaled_samples), size=n_samples, replace=False, p=err_eq_normalized)
    selected_samples = scaled_samples[selected_ids]
