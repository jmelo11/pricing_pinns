...
class DimlessBS(Closure):
    ...
    def compute_derivatives(self, x) -> tuple:
        ...
        for j in range(self.n_assets):
            grad_j = torch.autograd.grad(u_x[:, j].sum(), x, create_graph=True,retain_graph=True)[0][:, :self.n_assets]
    ...
    def boundary_loss(self, u, u_tau, u_x, u_xx) -> list[torch.Tensor]:
        ...
        for i in range(self.n_assets):
            mask_top_i = self.mask[:, 2 + 2*i].squeeze().bool()
            mask_bottom_i = self.mask[:, 3 + 2*i].squeeze().bool()
    ...
    def compute_losses(self):
        ...
        interior_mask = self.mask[:, 0].squeeze().bool()
        ...
        initial_mask = self.mask[:, 1].squeeze().bool()
